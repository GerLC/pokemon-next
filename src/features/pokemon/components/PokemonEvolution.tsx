...
import { useEvolutionChain } from "@/features/pokemon/hooks/use-pokemon";
import type { PokemonEvolutionProps } from "../types";
...
export function PokemonEvolution({ name }: PokemonEvolutionProps) {
  ...
  if (!chain || chain.length <= 1) {
    return (
      <div className="col-span-1 lg:col-span-2 bg-surface-card border border-border rounded-[var(--radius-card)] p-6 flex flex-col gap-2 items-center justify-center min-h-[200px]">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-4 w-full">
        {chain.map((evo, index) => (
          ...
          >
            ...
            >
              <div
                className={`w-32 h-32 md:w-40 md:h-40 relative rounded-full bg-surface border-2 ${evo.name === name ? "border-primary" : "border-border group-hover:border-primary/50"} flex items-center justify-center p-4 transition-colors`}
              >
                <Image
                  src={evo.artwork}
                  alt={evo.name}
              ...
              <span
                className={`font-bold capitalize ${evo.name === name ? "text-primary" : "text-on-surface group-hover:text-primary"}`}
              >
                {evo.name}
              </span>
            </Link>
